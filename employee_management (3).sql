...
--
-- Host: 127.0.0.1
-- Generation Time: Aug 13, 2026 at 04:48 PM
...
CREATE TABLE `attendance` (
  ...
  `att_status` enum('UnPaid','Paid') DEFAULT 'UnPaid'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TABLE `employees` (
  ...
  `emp_position` varchar(50) NOT NULL DEFAULT 'employee',
...
CREATE TABLE `payroll` (
  ...
  `pay_total_days` decimal(5,2) NOT NULL DEFAULT 0.00,
  `pay_status` enum('Unpaid','Paid','Cancelled') NOT NULL DEFAULT 'Unpaid',
...
ALTER TABLE `attendance`
  ...
  ADD UNIQUE KEY `unique_emp_daily_attendance` (`emp_id`,`att_work_date`);
...
ALTER TABLE `payroll`
  ...
  ADD KEY `fk_payroll_schedule` (`schedule_id`);

--
-- Indexes for table `payroll_schedule`
--
ALTER TABLE `payroll_schedule`
...
--
-- AUTO_INCREMENT for table `payroll_schedule`
--
ALTER TABLE `payroll_schedule`
...
--
ALTER TABLE `accounts`
  ADD CONSTRAINT `fk_accounts_employees` FOREIGN KEY (`emp_id`) REFERENCES `employees` (`emp_id`) ON DELETE CASCADE ON UPDATE CASCADE;
...
ALTER TABLE `payroll`
  ...
  ADD CONSTRAINT `fk_payroll_schedule` FOREIGN KEY (`schedule_id`) REFERENCES `payroll_schedule` (`schedule_id`);
COMMIT;
